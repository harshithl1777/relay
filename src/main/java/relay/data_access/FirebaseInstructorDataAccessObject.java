...
public class FirebaseInstructorDataAccessObject {
	...
	public Instructor read(String instructorID) {
		ApiFuture<DocumentSnapshot> future = FirestoreSingleton.get().collection("instructors").document(instructorID)
				.get();
	...
	public boolean delete(String instructorID) {
		...
		ApiFuture<WriteResult> deleteResult = FirestoreSingleton.get().collection("instructors").document(instructorID)
				.delete();
		try {
			deleteResult.get();
			return true;
